"""
Quick verification script for text segmentation and extraction features
"""
import os
import sys
import django
...
def test_segmentation():
    ...
    print(f"✓ Paragraph segmentation: {len(paragraphs)} paragraphs")
    ...
    print(f"✓ Semantic segmentation: {len(semantic)} sections")

    # Test with metadata
    text = "第一段\n\n第二段"
    with_metadata = processor.segment_text(text, mode="paragraph", include_metadata=True)
    print(f"✓ Segmentation with metadata: {len(with_metadata)} items with 'text', 'type', 'position'")
...
def test_extraction_templates():
    ...
    print("✓ All predefined templates present")

    # Check template structure
    assert isinstance(templates['cause_process_result'], list)
    assert len(templates['cause_process_result']) == 3
    print(f"✓ Template structure valid: {templates['cause_process_result']}")
...
def test_helper_methods():
    ...
    print(f"✓ Sentence segmentation helper: {len(sentences)} sentences")
    ...
    print("✓ Invalid content detection works")
